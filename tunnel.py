...
def start_tunnel():
    ...
    if ngrok_token:
        ...
        try:
            # Check if ngrok is in path or needs install
            if subprocess.run("command -v ngrok", shell=True, capture_output=True).returncode != 0:
    ...
    if not final_url:
        ...
        for fb in fallbacks:
            print(f"📡 Trying {fb['name']} fallback...")
    ...
    # 3. Send Final Message to Telegram
    if final_url:
        # POINTING TO vnc_lite.html TO BYPASS UI.JS ERRORS (Ctrl is null, etc.)
        vnc_url = f"{final_url}/vnc_lite.html?password={vnc_pass}"

        msg = (
            "🖥️ **Interactive RDP Console**\n"
            "━━━━━━━━━━━━━━━━━━━━━\n"
            f"🔗 [Open Interactive RDP]({vnc_url})\n\n"
            "⚠️ **Note:** If you see an 'Ngrok Warning', click **'Visit Site'**. This page only appears once.\n\n"
            "**Next Steps:**\n"
            "1. Open the link and Join the meeting manually.\n"
            "2. Once joined, return to Telegram and click **'Start Recording'**.\n"
            "3. RDP will close automatically when recording starts."
        )
        send_telegram(bot_token, chat_id, msg)
    else:
        send_telegram(bot_token, chat_id, "❌ **Connection Error:** Failed to establish any secure tunnel.")
